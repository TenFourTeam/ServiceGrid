...
export default function EmailSenderSettings() {
  ...
  const onConnect = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("nylas-auth-start", { body: { redirect_uri } });
    if (error) {
    ...
    <Card id="email-sending">
      ...
      <CardContent className="space-y-3">
        <div className="space-y-2">
          <p className="text-sm">
            {hasNylas
              ? `Connected via Nylas: ${sender?.from_email}`
              : "Send from your own mailbox by connecting it with Nylas (recommended), or set up SendGrid below."}
          </p>
