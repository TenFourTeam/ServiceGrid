...
export default function CreateQuoteModal({ open, onOpenChange, customers, defaultTaxRate = 0.1, onRequestSend }: CreateQuoteModalProps) {
  const { business, businessTaxRateDefault, businessId } = useBusinessContext();
  ...
  useEffect(() => {
    if (open) {
      ...
      setDraft({
        ...
        address: "",
        lineItems: [],
        taxRate: businessTaxRateDefault ?? defaultTaxRate,
        discount: 0,
        notesInternal: "",
        terms: "",
        paymentTerms: "due_on_receipt",
  ...
  }, [open, businessTaxRateDefault, defaultTaxRate]);
