...
-- Policies: users can manage their own domains
CREATE POLICY IF NOT EXISTS "Users can view their own email_domains" ON public.email_domains
FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own email_domains" ON public.email_domains
FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own email_domains" ON public.email_domains
FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own email_domains" ON public.email_domains
