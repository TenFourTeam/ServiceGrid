...
create policy if not exists "Mail sends are viewable by owner"
  on public.mail_sends for select
  using (auth.uid() = user_id);

create policy if not exists "Owner can insert their own mail sends"
  on public.mail_sends for insert
  with check (auth.uid() = user_id);

create policy if not exists "Owner can update their own mail sends"
