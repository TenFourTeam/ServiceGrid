...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { title, customerId, status, startsAt, endsAt, address, latitude, longitude, notes, jobType, quoteId, isAssessment, requestId, photos } = body;
    ...
    if (req.method === 'PUT') {
      ...
      const { id, title, status, startsAt, endsAt, address, notes, photos, isClockedIn, clockInTime, clockOutTime, isAssessment, requestId, quoteId, jobType } = body;
