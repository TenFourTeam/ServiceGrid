...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[Google Drive Export Quote] Exporting quote:', quoteId);
    ...
    const mockDriveFileId = '1QUOTE' + crypto.randomUUID().slice(0, 8);
    ...
    await supabase.from('google_drive_file_mappings').insert({
      ...
      sg_entity_type: 'quote',
      sg_entity_id: quoteId,
      drive_file_id: mockDriveFileId,
      drive_file_name: `Quote-${quote.number}.pdf`,
    ...
    const result = {
      ...
      message: 'Quote will be exported to Google Drive when API credentials are configured',
    };

    console.log('[Google Drive Export Quote] Completed:', result);
    ...
    console.error('[Google Drive Export Quote Error]:', error);
